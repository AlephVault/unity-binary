...
namespace AlephVault.Unity.Binary
{
    ...
    public class Buffer : Stream
    {
        ...
        public Buffer(int capacity, float growthFactor)
        {
        ...
        /// </summary>
        /// <param name="growthFactor">Factor by which buffer should grow when necessary.</param>
        public Buffer(float growthFactor) : this(k_InitialCapacity, growthFactor) { }

        /// <summary>
        /// A buffer that supports writing data smaller than a single byte. This buffer also has a built-in compression algorithm that can (optionally) be used to write compressed data.
        /// </summary>
        /// <param name="capacity"></param>
        public Buffer(int capacity) : this(capacity, k_InitialGrowthFactor) { }

        /// <summary>
        /// A buffer that supports writing data smaller than a single byte. This buffer also has a built-in compression algorithm that can (optionally) be used to write compressed data.
        /// </summary>
        public Buffer() : this(k_InitialCapacity, k_InitialGrowthFactor) { }
        ...
        /// </summary>
        public override long Length { get => Arithmetic.Div8Ceil(BitLength); }
        ...
        */
        ...
        public override long Seek(long offset, SeekOrigin origin)
        {
            return (long)((
                ...
                (
                    ...
                    (offset ^ long.MinValue) > Position ? 0UL :
                    BitPosition - (ulong)((offset ^ long.MinValue) << 3) :
        ...
        private void SetCapacity(long value)
        {
            ...
            System.Buffer.BlockCopy(m_Target, 0, newTarg, 0, (int)len);
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            if (BitAligned)
            {
                ...
                System.Buffer.BlockCopy(buffer, offset, m_Target, (int)Position, count);
                Position += count;
            }
            else
            {
        ...
        {
            if (Arithmetic.Div8Ceil(BitPosition) == m_Target.LongLength) Grow(1);
            if (BitAligned)
            {
                m_Target[Position] = value;
                BitPosition += 8;
        ...
        {
            if (s is Buffer b) Write(b.m_Target, 0, count < 0 ? (int)b.Length : count);
            else
            {
    ...
    public void CopyTo(Stream stream, int count = -1)
        ...
        public void CopyFrom(Buffer buffer, int dataCount, bool copyBits)
        {
            if (!copyBits)
            {
        ...
        public byte[] ToArray()
        {
            ...
            System.Buffer.BlockCopy(m_Target, 0, copy, 0, (int)Length);
            return copy;
        }
