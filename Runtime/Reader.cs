...
namespace AlephVault.Unity.Binary
{
    ...
    /// <summary>
    ///   <para>
    ///     A BinaryReader that can do bit wise manipulation when backed by a Buffer
    ///   </para>
    ///   <para>
    ...
    public class Reader
    {
        ...
        private Buffer m_NetworkSource;

        /// <summary>
        /// Creates a new Reader backed by a given stream
        /// </summary>
        /// <param name="stream">The stream to read from</param>
        public Reader(Stream stream)
        {
            m_Source = stream;
            m_NetworkSource = stream as Buffer;
        }
        ...
        public bool ReadBit()
        {
            ...
            {
                throw new InvalidOperationException($"Cannot read bits on a non-{nameof(Buffer)} stream");
            }
        ...
        public object ReadObjectPacked(Type type)
        {
            ...
            if (typeof(ISerializable).IsAssignableFrom(type))
            {
                ...
                ((ISerializable)instance).Serialize(new Serializer(this));
                return instance;
            }

            throw new ArgumentException($"{nameof(Reader)} cannot read type {type.FullName}");
        }
        ...
        public float ReadSingle() => new Float4 { UIntValue = ReadUInt32() }.FloatValue;

        /// <summary>
        /// Read a double-precision floating point value from the stream.
        /// </summary>
        /// <returns>The read value</returns>
        public double ReadDouble() => new Float8 { ULongValue = ReadUInt64() }.DoubleValue;

        /// <summary>
        /// Read a single-precision floating point value from the stream from a varint
        /// </summary>
        /// <returns>The read value</returns>
        public float ReadSinglePacked() => new Float4 { UIntValue = ReadUInt32Packed() }.FloatValue;

        /// <summary>
        /// Read a double-precision floating point value from the stream as a varint
        /// </summary>
        /// <returns>The read value</returns>
        public double ReadDoublePacked() => new Float8 { ULongValue = ReadUInt64Packed() }.DoubleValue;
        ...
        {
            if (m_NetworkSource == null) throw new InvalidOperationException($"Cannot read bits on a non-{nameof(Buffer)} stream");
