...
namespace AlephVault.Unity.Binary
{
    ...
    public class Writer
    {
        ...
        public void WriteObjectPacked(object value)
        {
            ...
            if (isNull || Classes.IsNullable(value.GetType()))
            {
                WriteBool(isNull);
            ...
            {
                ((ISerializable)value).Serialize(new Serializer(this));
            }
            else
            {
                throw new ArgumentException($"{nameof(Writer)} cannot write type {value.GetType().FullName}");
            }
        }
        ...
        {
            if (m_NetworkSink == null) throw new InvalidOperationException($"Cannot write bits on a non-{nameof(Buffer)} stream");
        ...
        /// <param name="targetSize"></param>
        public void ReadAndWrite(Reader sourceReader, long targetSize)
        {
            long TargetSize = targetSize;
            long LargeInt64Blocks = TargetSize >> 3; //Divide by 8
